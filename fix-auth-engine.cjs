...
engine = engine.replace(
  /static async createJob\(/g,
  "static async createJob(supabase: SupabaseClient, ",
);
engine = engine.replace(
  /static async processJob\(/g,
  "static async processJob(supabase: SupabaseClient, ",
);
engine = engine.replace(
  /static async processResult\(result: NormalizedBrandResult, runId: string\)/g,
  "static async processResult(supabase: SupabaseClient, result: NormalizedBrandResult, runId: string)",
);
engine = engine.replace(
  /await this\.processResult\(result, run\.id\);/g,
...
actions = actions.replace(
  /import \{ ResearchEngine \} from "\.\/engine";/g,
...
actions = actions.replace(
  /const job = await ResearchEngine\.createJob\(payload\.workspaceId,/g,
...
actions = actions.replace(
  /ResearchEngine\.processJob\(job\.id\)/g,
  "ResearchEngine.processJob(supabase, job.id)",
);
