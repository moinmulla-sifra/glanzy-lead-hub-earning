...
async function test() {
  ...
  if(brand) {
    // Create a product
    const { data: product, error: productErr } = await supabase.from('brand_products').insert({
      brand_id: brand.id,
      name: 'Test Product'
    }).select().single();
    ...
    // Check outreach
    const { error: outErr } = await supabase.from('outreach').select('contacted_by, contact_channel').limit(1);
