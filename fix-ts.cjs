...
apify = apify.replace(/process\.env\.APIFY_API_TOKEN/g, "process.env['APIFY_API_TOKEN']");
fs.writeFileSync('src/lib/research/providers/apify.ts', apify);

// tinyfish.ts
let tinyfish = fs.readFileSync('src/lib/research/providers/tinyfish.ts', 'utf8');
tinyfish = tinyfish.replace(/process\.env\.TINYFISH_API_KEY/g, "process.env['TINYFISH_API_KEY']");
fs.writeFileSync('src/lib/research/providers/tinyfish.ts', tinyfish);

// supabase.ts
let supabase = fs.readFileSync('src/lib/supabase.ts', 'utf8');
supabase = supabase.replace(/import\.meta\.env\.VITE_SUPABASE_URL/g, "import.meta.env['VITE_SUPABASE_URL']");
supabase = supabase.replace(/import\.meta\.env\.VITE_SUPABASE_ANON_KEY/g, "import.meta.env['VITE_SUPABASE_ANON_KEY']");
supabase = supabase.replace(/import\.meta\.env\.VITE_SUPABASE_PUBLISHABLE_KEY/g, "import.meta.env['VITE_SUPABASE_PUBLISHABLE_KEY']");
...
outreach = outreach.replace(/defaultSelectedId: string \| undefined;/g, "defaultSelectedId?: string | null;");
