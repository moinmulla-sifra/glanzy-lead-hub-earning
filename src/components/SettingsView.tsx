...
import { supabase, type Profile, type Workspace, type WorkspaceMember } from "@/lib/supabase";
  ...
  return (
    <div className="flex flex-col lg:flex-row h-full gap-8 pb-12 animate-in fade-in duration-500 max-w-6xl mx-auto w-full">
      ...
      <div className="lg:w-64 shrink-0 flex flex-col gap-2">
        <h1 className="text-3xl font-bold tracking-tight text-foreground mb-4 px-2">Settings</h1>
      ...
      <div className="flex-1 bg-card border border-border/60 rounded-3xl p-6 lg:p-10 subtle-shadow min-h-[500px]">
        ...
        {activeTab === "account" && (
          <div className="space-y-8 animate-in fade-in">
            <div>
              <h2 className="text-2xl font-bold text-foreground mb-1">Account</h2>
              <p className="text-muted-foreground text-sm">Manage your personal profile details.</p>
            </div>
            ...
            <div className="flex items-center gap-6">
              ...
              <div className="flex flex-col gap-1">
                <span className="text-lg font-bold text-foreground">{profile?.full_name || "Anonymous User"}</span>
                <span className="text-sm text-muted-foreground">User ID: {profile?.id}</span>
            ...
            <div className="border-t border-border/50 pt-8 flex items-center justify-between">
              <div>
                <h3 className="font-semibold text-foreground">Creator Profile</h3>
                <p className="text-sm text-muted-foreground max-w-md">Update your niche, connected platforms, and bio used for brand matching.</p>
              </div>
              <Link to="/profile" className="px-5 py-2.5 bg-foreground text-background font-semibold rounded-xl text-sm hover:bg-foreground/90 transition-colors shadow-sm">
                Edit Profile
              </Link>
            </div>
          </div>
        ...
        {activeTab === "workspace" && (
          <div className="space-y-8 animate-in fade-in">
            <div>
              <h2 className="text-2xl font-bold text-foreground mb-1">Workspace</h2>
              <p className="text-muted-foreground text-sm">Manage shared configuration for your team or agency.</p>
            </div>
            ...
            <div className="space-y-5 max-w-lg">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-muted/30 p-4 rounded-xl border border-border/50">
                  ...
                  <p className="font-medium capitalize">{workspaceInfo?.workspace_type || "Creator"}</p>
                </div>
                <div className="bg-muted/30 p-4 rounded-xl border border-border/50">
                  <p className="text-xs text-muted-foreground font-semibold uppercase mb-1">Your Role</p>
                  <p className="font-medium capitalize">{memberInfo?.role || "Member"}</p>
                </div>
              </div>
              ...
              {isWorkspaceAdmin && (
                ...
                >
                  {updateWorkspaceMutation.isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" />}
                  Save Workspace
                </button>
              )}
            </div>
            ...
            {profile?.account_type === "agency" && (
              <div className="border-t border-border/50 pt-8 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground max-w-md">Invite members and manage permissions for your agency.</p>
                </div>
                <button disabled className="px-5 py-2.5 bg-muted text-muted-foreground font-semibold rounded-xl text-sm opacity-50 cursor-not-allowed">
                  Coming Soon
                </button>
              </div>
            )}
        ...
        {activeTab === "appearance" && (
          <div className="space-y-8 animate-in fade-in">
            <div>
              ...
              <p className="text-muted-foreground text-sm">Customize how Branzly looks on your device.</p>
            </div>
        ...
        {activeTab === "security" && (
          <div className="space-y-8 animate-in fade-in">
            <div>
              ...
              <p className="text-muted-foreground text-sm">Manage your password and session.</p>
            </div>
            ...
            <div className="max-w-md space-y-4">
              ...
              >
                {updatePasswordMutation.isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : <Key className="w-4 h-4" />}
                Update Password
              </button>
            </div>

            <div className="border-t border-border/50 pt-8">
              <h3 className="font-semibold text-foreground mb-4">Active Session</h3>
              <button
        ...
        {activeTab === "notifications" && (
          <div className="space-y-8 animate-in fade-in">
            <div>
              ...
              <p className="text-muted-foreground text-sm">Control when and how you are contacted.</p>
            </div>

            <div className="space-y-6 max-w-lg">
              <div className="flex items-center justify-between p-4 bg-muted/30 rounded-xl border border-border/50">
                <div>
                  <h4 className="font-semibold text-sm">Recommendation Alerts</h4>
                  <p className="text-xs text-muted-foreground mt-0.5">Get notified about strong new brand matches.</p>
                </div>
              ...
              <div className="flex items-center justify-between p-4 bg-muted/30 rounded-xl border border-border/50">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">Receive news about Branzly features.</p>
                </div>
                <div className="w-10 h-6 bg-brand rounded-full relative opacity-50 cursor-not-allowed">
                  <div className="w-4 h-4 bg-white rounded-full absolute right-1 top-1 shadow-sm"></div>
                </div>
        ...
        {activeTab === "subscription" && (
          <div className="space-y-8 animate-in fade-in">
            <div>
              <h2 className="text-2xl font-bold text-foreground mb-1">Subscription</h2>
              <p className="text-muted-foreground text-sm">Manage your billing and plan limits.</p>
            </div>
            ...
            <div className="max-w-md bg-gradient-to-br from-brand/5 to-muted/20 border border-brand/20 rounded-2xl p-6 shadow-sm">
              <div className="flex items-center justify-between mb-6">
                <div>
                  <p className="text-xs font-bold text-brand uppercase tracking-wider mb-1">Current Plan</p>
              ...
              <div className="space-y-3 mb-6">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium text-foreground">Unlimited (Beta)</span>
                </div>
                <div className="flex justify-between text-sm">
              ...
              <button disabled className="w-full py-2.5 bg-foreground text-background font-semibold rounded-xl text-sm shadow-sm opacity-50 cursor-not-allowed">
        ...
        {activeTab === "danger" && (
          <div className="space-y-8 animate-in fade-in">
            <div>
              <h2 className="text-2xl font-bold text-destructive mb-1">Danger Zone</h2>
            ...
            <div className="max-w-lg border border-destructive/20 bg-destructive/5 rounded-2xl p-6">
              ...
              <button disabled className="px-5 py-2.5 bg-destructive/10 text-destructive font-semibold rounded-xl text-sm border border-destructive/20 opacity-50 cursor-not-allowed">
