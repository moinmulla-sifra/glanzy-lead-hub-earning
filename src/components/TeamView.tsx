      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex flex-col gap-2">
          ...
          <p className="text-muted-foreground text-lg">
            Manage access to your agency workspace ({workspaceMemberQuery.data?.workspaces?.name}).
          </p>
        </div>
      </div>
            ...
            {teamMembersQuery.isLoading ? (
              ...
              <div className="space-y-4">
                {teamMembersQuery.data?.map((member) => (
                  <div key={member.id} className="flex items-center justify-between p-4 rounded-2xl bg-muted/30 border border-border/50">
                    <div className="flex items-center gap-4">
                      <div className="w-10 h-10 rounded-full bg-background border border-border flex items-center justify-center overflow-hidden">
                        {member.profiles?.avatar_url ? (
                          ...
                          <span className="font-bold text-sm">{member.profiles?.full_name?.charAt(0) || "U"}</span>
                        )}
                      </div>
                      <div>
                        <p className="font-semibold text-foreground text-sm">{member.profiles?.full_name || "Unknown User"}</p>
                        <p className="text-xs text-muted-foreground capitalize">{member.role}</p>
                      </div>
                    </div>
                    
                    {isOwner && member.user_id !== userId && (
                      <button className="p-2 text-muted-foreground hover:text-destructive hover:bg-destructive/10 rounded-lg transition-colors" title="Remove Member">
                        <UserMinus size={18} />
                      </button>
                    )}
                  </div>
        ...
        <div className="space-y-4">
          <div className="bg-card border border-border/60 rounded-3xl p-6 subtle-shadow">
            ...
            >
              ...
              >
                {inviteMemberMutation.isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : <UserPlus className="w-4 h-4" />}
                Send Invitation
              </button>
