...
import { Loader2, Search, AlertCircle, CheckCircle2, PlayCircle, Clock , BarChart, Activity } from "lucide-react";
...
export function AdminResearchView() {
  ...
  if (jobsQuery.isLoading) {
    return <div className="flex items-center justify-center p-12"><Loader2 className="w-8 h-8 animate-spin text-brand" /></div>;
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        ...
        <span className="text-sm text-muted-foreground">{jobs.length} recent jobs</span>
      </div>
            ...
            <tbody className="divide-y divide-border">
              {jobs.map((job) => (
                <tr key={job.id} className="hover:bg-muted/50 transition-colors">
                  <td className="px-6 py-4 font-mono text-xs" title={job.id}>{job.id.substring(0, 8)}...</td>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2">
                      ...
                      {job.status === 'running' && <PlayCircle className="w-4 h-4 text-brand animate-pulse" />}
                      {job.status === 'queued' && <Clock className="w-4 h-4 text-muted-foreground" />}
                      <span className="capitalize">{job.status}</span>
                    </div>
                    {job.error && <div className="text-xs text-destructive mt-1 max-w-xs truncate" title={job.error}>{job.error}</div>}
                  </td>
                  <td className="px-6 py-4">{job.result_count}</td>
                  <td className="px-6 py-4 text-muted-foreground">{new Date(job.created_at).toLocaleString()}</td>
                </tr>
              ))}
              {jobs.length === 0 && (
                <tr>
