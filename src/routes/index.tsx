...
import { Compass, Mail, BarChart, ArrowRight, CheckCircle2 } from "lucide-react";
    ...
    <div className="min-h-screen bg-background selection:bg-brand/20">
      <header className="fixed top-0 left-0 right-0 h-16 border-b border-border/50 bg-background/80 backdrop-blur-xl z-50 flex items-center justify-between px-6 max-w-7xl mx-auto">
        ...
        <nav className="flex items-center gap-4">
          ...
          <Link to="/auth" search={{ mode: "signup" }} className="text-sm font-semibold bg-foreground text-background px-4 py-2 rounded-lg hover:bg-foreground/90 transition-colors">
            Get Started
          </Link>
        </nav>
      </header>
      ...
      <main className="pt-32 pb-24 px-6 max-w-7xl mx-auto">
        <div className="text-center max-w-3xl mx-auto mb-20 animate-in fade-in slide-in-from-bottom-4 duration-700">
          ...
          <h1 className="text-5xl lg:text-7xl font-bold tracking-tight text-foreground mb-6 leading-tight">
            The CRM built for the <span className="text-brand">creator economy</span>.
          </h1>
          <p className="text-xl text-muted-foreground mb-10 leading-relaxed">
            Discover perfectly matched brands, track your outreach pipeline, and close more sponsorships without the spreadsheet chaos.
          </p>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <Link to="/auth" search={{ mode: "signup" }} className="w-full sm:w-auto flex items-center justify-center gap-2 bg-brand text-brand-foreground font-semibold rounded-xl px-8 py-4 text-lg hover:bg-brand/90 transition-all shadow-lg shadow-brand/20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-5xl mx-auto">
          <div className="bg-card border border-border/50 rounded-3xl p-8 subtle-shadow text-center">
            ...
            <p className="text-muted-foreground">Find brands actively looking for creators in your specific niche with our dynamic opportunity engine.</p>
          </div>
          <div className="bg-card border border-border/50 rounded-3xl p-8 subtle-shadow text-center">
            <div className="w-14 h-14 rounded-2xl bg-brand/10 flex items-center justify-center mx-auto mb-6">
            ...
            <p className="text-muted-foreground">Track every conversation from initial pitch to signed contract in a beautiful Kanban board designed for deals.</p>
          </div>
          <div className="bg-card border border-border/50 rounded-3xl p-8 subtle-shadow text-center">
            <div className="w-14 h-14 rounded-2xl bg-brand/10 flex items-center justify-center mx-auto mb-6">
            ...
            <p className="text-muted-foreground">Manage your entire roster's pipeline with team workspaces, shared contacts, and aggregated reporting.</p>
          </div>
        </div>
      </main>
      ...
      <footer className="border-t border-border/50 py-12 px-6">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-2 opacity-50">
            ...
            <span className="font-bold text-sm">Branzly © {new Date().getFullYear()}</span>
          </div>
