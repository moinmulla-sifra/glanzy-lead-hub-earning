...
export class ResearchEngine {
    ...
    if (existingBrands && existingBrands.length > 0) {
      ...
      const { data: newBrand, error } = await supabase
        ...
        .insert({
          ...
          normalized_name: (result.company_name || "")
            .toLowerCase()
            .replace(/[^a-z0-9]/g, ""),
  ...
  static async runAutomatedHourlyJob(supabase: SupabaseClient) {
    ...
    if (!queueItem) {
      ...
      await supabase.from("research_queue").insert(defaults);
    ...
    try {
      ...
      if (!provider) throw new Error("Provider not found");
      const { results, raw } = await provider.discoverBrands(
        queueItem.research_query,
      );
      ...
      console.log(
        `Automated run complete. Found ${newBrandsCount} new leads (Target: ${targetPerRun}, Duplicates: ${duplicates}, Rejected: ${rejected})`,
      );
      return {
        status: "completed",
