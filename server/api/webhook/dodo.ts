...
export const handleDodoWebhook = async (request: Request) => {
  ...
  if (!payload || !signature) {
    return new Response(JSON.stringify({ message: "Missing payload or signature" }), { status: 400, headers: { "Content-Type": "application/json" } });
  }
  ...
  try {
    ...
    if (existingEvent) {
      return new Response(JSON.stringify({ received: true, message: "Duplicate event ignored" }), { headers: { "Content-Type": "application/json" } });
    }
    ...
    if (parsed.data && parsed.data.metadata && parsed.data.metadata.workspace_id) {
      ...
      } else if (eventType === "subscription.canceled" || eventType === "subscription.expired" || eventType === "subscription.failed") {
        await supabase
          .from("subscriptions")
          .update({
