...
export const handleDiscover = async (request: Request) => {
  try {
    ...
    if (pageParam === 0 && search) {
      if (planConfig.limits.daily_brand_searches !== "unlimited" && searches >= planConfig.limits.daily_brand_searches) {
        return new Response(JSON.stringify({ error: "Daily search limit reached" }), { status: 403, headers: { "Content-Type": "application/json" } });
      }
      searches++;
    }

    if (planConfig.limits.daily_brand_leads !== "unlimited" && leads >= planConfig.limits.daily_brand_leads) {
      return new Response(JSON.stringify({ error: "Daily lead limit reached" }), { status: 403, headers: { "Content-Type": "application/json" } });
    }
    ...
    if (search) {
      q = q.or(`company_name.ilike.%${search}%,industry.ilike.%${search}%,country.ilike.%${search}%`);
    }
    ...
    return new Response(JSON.stringify({ brands: maskedBrands, count, nextPage: (pageParam + 1) * pageSize < (count || 0) ? pageParam + 1 : undefined }), { headers: { "Content-Type": "application/json" } });
  } catch (err: any) {
