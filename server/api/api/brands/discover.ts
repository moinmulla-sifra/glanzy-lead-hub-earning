...
export const APIRoute = createAPIFileRoute('/api/brands/discover')({
  POST: async ({ request }) => {
    ...
    if (pageParam === 0 && search) {
      if (planConfig.limits.daily_brand_searches !== "unlimited" && searches >= planConfig.limits.daily_brand_searches) {
        return new Response(JSON.stringify({ error: "Daily search limit reached" }), { status: 403 });
      }
      searches++;
    }

    if (planConfig.limits.daily_brand_leads !== "unlimited" && leads >= planConfig.limits.daily_brand_leads) {
      return new Response(JSON.stringify({ error: "Daily lead limit reached" }), { status: 403 });
    }
