...
//#region node_modules/.nitro/vite/services/ssr/assets/actions-BwGTXDv0.js
...
var startResearchJob = createServerFn({ method: "POST" }).validator((d) => d).handler(startResearchJob_createServerFn_handler, async ({ data: payload }) => {
	const supabase = createClient(processModule.env["VITE_SUPABASE_URL"] || "https://ldxjxrtdylnuhvmmcveg.supabase.co", processModule.env["VITE_SUPABASE_ANON_KEY"] || "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImxkeGp4cnRkeWxudWh2bW1jdmVnIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODg2OTgxMjUsImV4cCI6MjEwNDI3NDEyNX0.C7mUyroSPQ7Vcpepiqv-jzSd-zhTB4fHuFrMX23l3HY", { global: { headers: { Authorization: `Bearer ${payload.token}` } } });
...
var triggerAutomatedResearch = createServerFn({ method: "POST" }).validator((d) => d).handler(triggerAutomatedResearch_createServerFn_handler, async ({ data: payload }) => {
	...
	const elevatedSupabase = serviceRoleKey ? createClient(processModule.env["VITE_SUPABASE_URL"] || "https://ldxjxrtdylnuhvmmcveg.supabase.co", serviceRoleKey) : supabase;
