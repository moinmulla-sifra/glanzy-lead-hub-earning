...
//#region node_modules/.nitro/vite/services/ssr/assets/pricing-bHBaTueO.js
...
function PricingPage() {
	...
	(0, import_react.useEffect)(() => {
		async function loadPlan() {
			...
			const { data: memberData } = await supabase.from("workspace_members").select("workspace_id").eq("user_id", session.session.user.id).limit(1).single();
			if (memberData) {
				setWorkspaceId(memberData.workspace_id);
				const { data: subData } = await supabase.from("subscriptions").select("plan").eq("workspace_id", memberData.workspace_id).single();
