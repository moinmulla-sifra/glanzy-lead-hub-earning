...
//#endregion
//#region node_modules/.pnpm/srvx@1.0.3/node_modules/srvx/dist/adapters/cloudflare.mjs
var FastURL = URL;
...
var NEEDLESS_ESCAPE_RE_G = /* @__PURE__ */ new RegExp(
  NEEDLESS_ESCAPE_SRC,
  "gi",
);
  ...
  if (!(val instanceof Response)) {
    ...
    return new FastResponse(
      nullBody(event.req.method, status) ? null : res.body,
      {
        status,
        statusText:
          rawStatusText === void 0
            ? void 0
            : sanitizeStatusMessage(rawStatusText),
        headers:
...
function defineHandler(input) {
  ...
  const handler =
    ...
    (input.fetch
      ...
      : NoHandler);
  const composed =
    input.middleware?.length && composeHandler(input.middleware, handler);
  const eventHandler = handlerWithFetch(composed || handler);
  return Object.assign(
    eventHandler,
    input,
    composed && { fetch: eventHandler.fetch },
  );
}
...
function createDispatcher(app) {
  if (app["~getMiddleware"] !== H3Core.prototype["~getMiddleware"])
    return (event, route) =>
      callMiddleware(
        event,
        app["~getMiddleware"](event, route || void 0),
        routeHandler(route),
      );
