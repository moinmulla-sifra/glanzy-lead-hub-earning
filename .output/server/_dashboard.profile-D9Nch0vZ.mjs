...
import { i as User, k as LoaderCircle, v as Save } from "./_libs/lucide-react.mjs";
import { n as toast } from "./_libs/sonner.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/_dashboard.profile-D9Nch0vZ.js
...
function ProfileView({ userId }) {
	...
	(0, import_react.useEffect)(() => {
		if (profileQuery.data) setFormData({
			...profileQuery.data,
	...
	const updateProfileMutation = useMutation({
		mutationFn: async (updates) => {
			...
			if (isAgency && updates.agency_name !== void 0) payload.agency_name = updates.agency_name;
			const { error } = await supabase.from("profiles").upsert(payload);
			if (error) throw error;
			if (isAgency && updates.agency_name) {
				const { data: wsMember } = await supabase.from("workspace_members").select("workspace_id").eq("user_id", userId).eq("role", "owner").maybeSingle();
				if (wsMember?.workspace_id) await supabase.from("workspaces").update({
