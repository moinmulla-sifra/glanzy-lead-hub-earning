...
import { r as o, x as s } from "./index-Difz8PWa.js";
import { a as c } from "./phone-CA_HO8J9.js";
import { t as l } from "./supabase-DTorYgRt.js";
import { t as u } from "./useMonetization-C0l42Clg.js";
import { t as d } from "./bookmark-BtRCrCO3.js";
import { n as f, t as p } from "./BrandProfileModal-CIbb-wAf.js";
import { t as m } from "./sparkles-C-wfMPnb.js";
import { t as h } from "./trending-up-9fTZhAs5.js";
var g = e(t(), 1),
  _ = n();
function v({ userId: e }) {
  let t = s(),
    [n, d] = (0, g.useState)(null),
    [v, b] = (0, g.useState)(new Set()),
    x = i({
      queryKey: [`profile`, e],
      enabled: !!e,
      queryFn: async () => {
    ...
    { workspaceId: S } = u(e);
  i({
    queryKey: [`saved_brands_set`, S],
    enabled: !!S,
    queryFn: async () => {
      let { data: e, error: t } = await l
        .from(`saved_brands`)
        .select(`brand_id`)
        .eq(`workspace_id`, S);
      if (t) throw t;
      let n = new Set(e.map((e) => e.brand_id));
      return (b(n), n);
    },
  });
  let C = i({
      queryKey: [`outreach_exclusion`, S],
      enabled: !!S,
      queryFn: async () => {
        let { data: e, error: t } = await l
          .from(`outreach`)
    ...
    T = a({
      mutationFn: async ({ brandId: e, isSaved: t }) => {
        if (!S) throw Error(`No workspace selected`);
        if (t) {
          let { error: t } = await l
            .from(`saved_brands`)
            .delete()
          ...
          let { error: t } = await l
            .from(`saved_brands`)
            .insert({ workspace_id: S, brand_id: e });
          if (t) throw t;
        }
      },
      onSuccess: (e, n) => {
        (b((e) => {
          let t = new Set(e);
          return (n.isSaved ? t.delete(n.brandId) : t.add(n.brandId), t);
        }),
          o.success(n.isSaved ? `Removed from Saved` : `Added to Saved`),
          t.invalidateQueries({ queryKey: [`saved_brands_set`] }),
          t.invalidateQueries({ queryKey: [`saved_brands`] }));
      },
      onError: (e) => o.error(e.message || `Failed to update saved status`),
    }),
    E = (0, g.useMemo)(() => {
      if (!x.data || !w.data) return [];
      let e = x.data,
        t = C.data || new Set(),
        n = [];
      for (let r of w.data) {
        ...
        let i = 0,
          a = [],
          o = e.niche || e.primary_niche;
        (o &&
          r.industry &&
          (r.industry.toLowerCase().includes(o.toLowerCase()) ||
          o.toLowerCase().includes(r.industry.toLowerCase())
            ? ((i += 40), a.push(`Strong match for your ${o} niche`))
            : r.industry === `General` && (i += 10)),
          e.country &&
            r.country &&
            e.country === r.country &&
            ((i += 20), a.push(`Based in your country`)),
          r.influencer_fit_score &&
            r.influencer_fit_score > 75 &&
            ((i += r.influencer_fit_score / 2),
            a.push(`High creator-fit score`)),
          r.recent_funding && ((i += 15), a.push(`Recently funded`)),
          r.recent_launch &&
            ((i += 15), a.push(`Recently launched a campaign`)),
          r.lead_score && (i += r.lead_score / 10),
          i > 30 && n.push({ brand: r, score: i, reasons: a }));
      }
      return n.sort((e, t) => t.score - e.score);
    }, [x.data, w.data, C.data]),
    D = x.isLoading || w.isLoading,
    O =
      x.isSuccess &&
      !x.data?.niche &&
      !x.data?.primary_niche &&
      !x.data?.platforms?.length;
  return (0, _.jsxs)(`div`, {
    ...
    children: [
      (0, _.jsxs)(`div`, {
        ...
        children: [
          (0, _.jsxs)(`h1`, {
            ...
            children: [
              (0, _.jsx)(m, { className: `text-brand`, size: 32 }),
              ` For You`,
            ],
          }),
          (0, _.jsx)(`p`, {
                  ...
                  children: [
                    (0, _.jsxs)(`section`, {
                      children: [
                        ...
                        (0, _.jsx)(`div`, {
                          className: `grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6`,
                          children: E.slice(0, 6).map((e) =>
                            (0, _.jsx)(
                              y,
                              {
                                rec: e,
                                isSaved: v.has(e.brand.id),
                                onToggleSave: () =>
                                  T.mutate({
                                    brandId: e.brand.id,
                                    isSaved: v.has(e.brand.id),
                                  }),
                                onClickView: () => d(e.brand),
                              },
                              e.brand.id,
                            ),
                          ),
      ...
      n &&
        ...
        (0, _.jsx)(p, {
          ...
          onSave: () => T.mutate({ brandId: n.id, isSaved: v.has(n.id) }),
          onStartOutreach: () => {},
        }),
    ],
  });
}
function y({ rec: e, isSaved: t, onToggleSave: n, onClickView: r }) {
  let { brand: i, score: a, reasons: o } = e,
    s = Math.min(99, Math.max(65, Math.floor(a)));
  return (0, _.jsxs)(`div`, {
      ...
      (0, _.jsxs)(`div`, {
        ...
        children: [
          ...
          (0, _.jsx)(`button`, {
            ...
            className: `p-2.5 rounded-xl border transition-colors ${t ? `bg-brand/10 border-brand/30 text-brand hover:bg-brand/20` : `bg-background border-border hover:bg-muted text-muted-foreground`}`,
            title: t ? `Remove from Saved` : `Save Brand`,
            children: (0, _.jsx)(d, {
              size: 20,
              className: t ? `fill-brand` : ``,
            }),
          }),
        ],
      }),
...
function b({ size: e, className: t }) {
  return (0, _.jsx)(`svg`, {
    ...
    className: t,
    children: (0, _.jsx)(`polyline`, { points: `20 6 9 17 4 12` }),
  });
}
function x() {
